    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter, (x_, _) in enumerate(self.data_loader):
                ...
                # need to scale x_ to be in similar range to g_ for this to work
                ...
                if ((iter + 1) % self.results_save_step) == 0:
                    ...
                    print(G_.size())
                    print(x_.size())
                    mu_f = G_.mean([2, 3]).data.cpu()
                    mu_r = x_.mean([2, 3]).data.cpu()
                    sigma_f = G_.view(-1,G_.size(1), G_.size(2)* G_.size(3)).std(2).data.cpu()
                    sigma_r = x_.view(-1, x_.size(1), x_.size(2) * x_.size(3)).std(2).data.cpu()
                    kid = polynomial_mmd_averages(mu_f, mu_r, subset_size=self.batch_size, ret_var=False)
                    # , output=sys.stdout, **kernel_args)
                    fid = calculate_frechet_distance(mu_f.numpy(), sigma_f.numpy(), mu_r.numpy(), sigma_r.numpy(), eps=1e-6)
                    i_s = None

                    self.results['kid'].append((iter, kid))
                    self.results['fid'].append((iter, fid))
                    self.results['is'].append((iter, i_s))
