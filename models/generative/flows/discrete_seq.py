...
import math
from models.networks.generative.flows.lstm import AFPrior
from models.layers.ff import FeedForwardNet
from utils.batchers import make_pos_cond


class InferenceBlock(nn.Module):
    def __init__(self, inf_inp_dim, hidden_size, zsize,
                 dropout_p, q_rnn_layers, dropout_locations, max_T):
        super().__init__()
        self.dropout = nn.Dropout(dropout_p)

        rnn_q_inp_size = inf_inp_dim + 2 * max_T
        self.rnn_q = torch.nn.LSTM(rnn_q_inp_size, hidden_size, q_rnn_layers,
                                   dropout=dropout_p if 'rnn_x' in dropout_locations else 0, bidirectional=True)
    ...
    def sample_q_z(self, inf_inp, lengths, cond_inp, ELBO_samples):
        ...
        log_q_z = -1 / 2 * (math.log(2 * math.pi) + z_base_logvar + (z - z_base_mean).pow(2) / z_base_std.pow(2)).sum(
            -1)  # [T, B, s]
...
class GenerativeBlock(nn.Module):
    def __init__(self, hidden_size, zsize, prior_type, dropout_p, dropout_locations, outp_rnn_layers, max_T, **kwargs):
        ...
        self.prior = AFPrior(p_rnn_units, zsize, dropout_p, dropout_locations, prior_type, p_num_flow_layers,
                             p_rnn_layers,
                             max_T=max_T, transform_function=transform_function, hiddenflow_params=hiddenflow_params)

        # BiLSTM
        self.rnn_outp = nn.LSTM(zsize + 2 * max_T, hidden_size, outp_rnn_layers,
                                dropout=dropout_p if 'rnn_outp' in dropout_locations else 0, bidirectional=True)
    ...
    def apply_bilstm(self, z, lengths_s, cond_inp_s):
        ...
        hidden_outp = tuple(
            h[:, :, None, :].repeat(1, 1, ELBO_samples, 1).view(-1, B * ELBO_samples, self.hidden_size) for h in
            hidden_outp)
        ...
        # Reorganize recurrent output
        hidden_outp_sep = hidden_outp[0].view(self.outp_rnn_layers, 2, B, ELBO_samples, self.hidden_size)

        rnn_outp_outp_shifted_forward = torch.cat((hidden_outp_sep[-1:, 0], rnn_outp_outp[:, :, :, 0]), 0)[
                                        :-1]  # [T, B, s, hidden]
...
class DFModel(nn.Module):
    def __init__(self, vocab_size, loss_weights, n_inp_embedding, hidden_size, zsize, dropout_p, dropout_locations,
                 # general parameters
        ...
        for loc in dropout_locations:
            if loc not in ['embedding', 'rnn_x', 'z_before_prior', 'prior_rnn_inp', 'prior_rnn', 'prior_ff',
                           'z_before_outp', 'rnn_outp', 'outp_ff']:
        ...
        ## Latent mods
        self.generative_model = GenerativeBlock(hidden_size, zsize, prior_type, dropout_p, dropout_locations,
                                                gen_bilstm_layers, max_T, **prior_kwargs)

        self.inference_model = InferenceBlock(n_inp_embedding, hidden_size, zsize, dropout_p, q_rnn_layers,
                                              dropout_locations, max_T)

        ## Generative output to x
        self.outp_ff = FeedForwardNet(self.generative_model.outp_dim, hidden_size, vocab_size, 1, 'none',
                                      dropout=dropout_p if 'outp_ff' in dropout_locations else 0)
    ...
    def evaluate_x(self, x, lengths, ELBO_samples=1):
        ...
        if self.indep_bernoulli:
            ...
            reconst_loss = self.cross_entropy(scores.view(-1, self.vocab_size), targets.view(-1, self.vocab_size)).view(
                T, B, ELBO_samples, self.vocab_size).sum(-1)  # [T, B, s]
        else:
            targets = x[:, :, None].repeat(1, 1, ELBO_samples)
            reconst_loss = self.cross_entropy(scores.view(-1, self.vocab_size), targets.view(-1)).view(T, B,
                                                                                                       ELBO_samples)
